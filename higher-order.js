...
//Code Here
let totalPopulation = populations.reduce((acc,value,index,array) => {
  return acc + value;
})
...
//Code Here
let myStrongest = monstersInYourPocket.filter((value,index,array) => {
  return value.CP > 200;
})
...
let orderTotals = orders.map((value) => {
  return value.price * (value.tax + 1);
})
...
let bobsTotal = purchases.filter((value) => {
  return value.owner === 'Bob';
